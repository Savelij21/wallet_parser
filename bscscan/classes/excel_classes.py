...
from bscscan.classes.wallet_classes import Wallet, Ticker
...
class ExcelWallet:
    ...
    def __filling_tickers_to_excel(self):
        ...
        def fill_total_string_for_ticker(ticker_obj: Ticker, string_num: int):  # only data, no styles
            ...
            # token symbol column
            self.sheet[f'A{string}'] = f'{ticker_obj.ticker_symbol}, {ticker_obj.full_ticker_name}'
        ...
        if self.wallet.show_details:  # if user wants to get transactions details
            for ticker in self.wallet.tickers:
                ...
                if (ticker.volume_24h_usd == 'N/A' or int(ticker.volume_24h_usd) < 300) and ticker.total_delta_usd > 0:
                    for i in range(1, 16):
            ...
            self.sheet.column_dimensions['B'].width = 1
            self.sheet.column_dimensions['J'].width = 1
            self.sheet.column_dimensions['K'].width = 1

            for ticker in self.wallet.tickers:
                # TOTAL DATA FOR TICKER
                # coloring string if token is SCAMMED (before data filling because usual data not need in styles)
                if (ticker.volume_24h_usd == 'N/A' or ticker.volume_24h_usd) < 300 and ticker.total_delta_usd > 0:
                    for i in range(1, 16):
                        if i == 9:  # skip sum_delta column to not paint this field to black
                            continue
                        self.sheet.cell(row=current_string, column=i).font = Font(color='FFFFFF')
